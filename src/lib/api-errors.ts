...
export function mapApiError(error: unknown): { status: number; body: ApiErrorBody } {
  ...
  if (error instanceof McpCapabilityError) {
    const mapping: Record<string, readonly [number, string]> = {
      ...
      MCP_CONNECTION_V2_ATTESTATION_DELETE_FORBIDDEN: [409, "MCP 连接仍有管理员认证历史，无法永久删除"],
      MCP_CONNECTION_DELETE_REQUIRES_DISABLED: [409, "请先停用 MCP 连接，再执行永久删除"],
      MCP_CONNECTION_CONFIRMATION_MISMATCH: [400, "连接名称确认不一致，未执行删除"],
      MCP_CONNECTION_GOVERNANCE_REQUIRED: [409, "连接高风险变更必须先完成安全影响预览与确认"],
      MCP_CONNECTION_PREVIEW_NOT_FOUND: [404, "连接变更预览不存在或不属于当前账户"],
      MCP_CONNECTION_PREVIEW_EXPIRED: [409, "连接变更预览已过期，请重新生成"],
      MCP_CONNECTION_PREVIEW_CONSUMED: [409, "连接变更预览已经消费，不能重复执行"],
      MCP_CONNECTION_PREVIEW_MISMATCH: [409, "连接变更预览与当前请求不一致，未执行操作"],
      MCP_CONNECTION_IMPACT_CHANGED: [409, "连接影响范围已变化，请重新预览"],
      MCP_EXTERNAL_IO_PLANNED_NOT_DISPATCHED: [409, "MCP 重发现或重信任尚未开放外部发包，未执行远程操作"],
      ...
      MCP_TOOL_REVIEW_REQUIRED: [409, "MCP 工具认证必须通过审核工作台创建，旧版认证入口已冻结"],
      MCP_ATTESTATION_NOT_FOUND: [404, "MCP 工具管理员认证不存在或已撤销"],
      MCP_ATTESTATION_CONFLICT: [409, "MCP 工具管理员认证已被其他管理员更新，请刷新后重试"],
      MCP_ATTESTATION_REAUTHENTICATION_REQUIRED: [409, "现有 MCP 工具认证已失效，请先撤销后重新认证"],
      MCP_TOOL_REVIEW_INVALID_INPUT: [400, "MCP 工具审核请求无效"],
      MCP_TOOL_REVIEW_NOTE_UNSAFE: [400, "审核说明包含凭据、地址或其他不允许的敏感内容"],
      MCP_TOOL_REVIEW_CANDIDATE_STALE: [409, "MCP 工具候选快照已变化，请刷新后重新审核"],
      MCP_TOOL_REVIEW_IDEMPOTENCY_CONFLICT: [409, "同一审核请求标识已经用于另一项审核"],
      MCP_TOOL_REVIEW_NOT_FOUND: [404, "MCP 工具审核记录不存在"],
      MCP_TOOL_REVIEW_ATTESTATION_CONFLICT: [409, "MCP 工具当前认证状态与审核结论冲突，请刷新后重试"],
  ...
  if (error instanceof GitServiceError) {
    const mapping: Record<string, readonly [number, string]> = {
      ...
      GIT_CONNECTION_GOVERNANCE_REQUIRED: [409, "连接高风险变更必须先完成安全影响预览与确认"],
      GIT_CONNECTION_PREVIEW_NOT_FOUND: [404, "连接变更预览不存在或不属于当前账户"],
      GIT_CONNECTION_PREVIEW_EXPIRED: [409, "连接变更预览已过期，请重新生成"],
      GIT_CONNECTION_PREVIEW_CONSUMED: [409, "连接变更预览已经消费，不能重复执行"],
      GIT_CONNECTION_PREVIEW_MISMATCH: [409, "连接变更预览与当前请求不一致，未执行操作"],
      GIT_CONNECTION_IMPACT_CHANGED: [409, "连接影响范围已变化，请重新预览"],
      GIT_EXTERNAL_IO_PLANNED_NOT_DISPATCHED: [409, "Git 重测或重信任尚未开放外部发包，未执行远程操作"],
  ...
  if (error instanceof ProjectDelegatedGitRuntimeError) {
    const mapping: Record<string, readonly [number, string]> = {
      PROJECT_GIT_MANUAL_INVALID_INPUT: [400, "手动 Git 读取请求无效"],
      PROJECT_GIT_MANUAL_NOT_FOUND: [404, "项目 Git 手动读取记录不存在"],
      PROJECT_GIT_MANUAL_FORBIDDEN: [403, "无权执行该项目 Git 手动读取"],
      PROJECT_GIT_MANUAL_MEMBERSHIP_REQUIRED: [403, "需要明确的项目编辑成员资格"],
      PROJECT_GIT_MANUAL_PROJECT_ARCHIVED: [409, "已归档项目不能执行 Git 手动读取"],
      PROJECT_GIT_MANUAL_INELIGIBLE: [409, "项目 Git 委托当前不可用于手动读取"],
    ...
    const [status, message] = mapping[error.code] ?? [500, "项目 Git 手动读取失败"];
    return { status, body: { error: { code: error.code, message } } };
  }

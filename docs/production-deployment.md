...
状态：`CONTROLLED_PRERELEASE`。当前批准的生产目标是 `v0.5.0-dev.3`。该版本仍是预发布，不是稳定版或 GitHub Latest；工作流只对这一精确标签开放，旧 `v0.5.0-dev.1` clean-reset 入口、`v0.5.0-dev.2`、`v0.4.0-dev.1`、`v0.3.0-dev.1`、`v0.2.0-dev.1` 和其他 `-dev` 标签继续失败关闭。
...
## v0.5.0-dev.3 preserve-data 边界

`v0.5.0-dev.3` 只允许从已运行的 `v0.5.0-dev.1` 做一次性保留数据切换。部署器在旧 app/worker 继续服务时仅构建这两个新镜像；停止精确旧 writer 后排空数据库客户端，完成 `pre-deploy-to-v0.5.0-dev.3` 加密异地备份，再只重建 app/worker。它不会运行数据库迁移、principal bootstrap、reconcile，不会删除或重建 PostgreSQL 容器/卷，也不会执行 Compose down 或 Docker prune。
...
- 当前输入只接受 `v0.5.0-dev.3`，目标必须是 annotated tag，且 `package.json` 版本必须精确匹配 `0.5.0-dev.3`；服务器同时固定核验不可变的 `.1` 源标签。
- 部署前会通过 GitHub API 确认 `.1` 源标签和 `.3` 目标标签对应精确提交的 `CI` push 运行已经 `completed/success`。
- GitHub 使用独立 ED25519 私钥；服务器对应公钥带 `restrict` 和 forced-command，不能获取 Shell、PTY、端口转发或执行任意命令。
- forced-command 只接受 `preserve-deploy v0.5.0-dev.1 v0.5.0-dev.3 <40 位 SHA> CONFIRM_PRESERVE_DATA_V1`，再调用 root 持有的固定部署程序；sudoers 不再开放 clean-deploy。专用系统账号 `ai-project-os-actions` 没有人工登录密钥；`deploy` 用户不加入 `docker` 组，也不获得无密码 sudo。
...
该命令需要服务器已经安装当前版本的受限网关、preserve-data 部署器和配置器。部署 `v0.5.0-dev.3` 前必须从受信候选源码重新运行 `install-production-deploy.sh`，使 root-owned 工具、106-entry migration ledger 门禁、完整容器 ID 字符串校验与精确 tag allowlist 同步更新；旧网关不接受该 preserve 命令。安装工具不会自动部署应用，配置 Environment 也不会绕过标签、CI、备份或数据不变性预检。
...
3. 点击 **Run workflow**，Branch 保持 `main`，确认 tag 为 `v0.5.0-dev.3`。
...
- 人工恢复前先确认目标版本与备份 manifest；`.1` recovery 可以接受 `pre-deploy-to-v0.5.0-dev.3` 备份，未来 `.3` recovery 只接受 appVersion 与目标标签一致的备份。

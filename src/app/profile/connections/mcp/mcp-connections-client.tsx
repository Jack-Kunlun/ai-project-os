...
function McpCreateForm({ onCreated }: { onCreated: (connection: McpConnection) => void }) {
  ...
  async function testConnection(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const probeResponse = await fetch("/api/me/mcp-connections/probe", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(draftInput) });

...
import { prepareProjectAgentConfirmation, runProjectAgentJob } from "@/lib/web-project-intelligence";
...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    if (body.phase === "prepare") {
      const confirmation = await prepareProjectAgentConfirmation({ projectId, requestedBy: user, clientKey: body.clientKey, question: body.question });
      return NextResponse.json({ confirmation }, { headers: { "cache-control": "no-store" } });
    }
    const job = await runProjectAgentJob({
      projectId,
      requestedBy: user,

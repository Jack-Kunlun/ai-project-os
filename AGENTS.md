...
这是一个独立的 Next.js 单体应用，V2.0 的代码、迁移、文档和测试均在本目录内。不要扩展到其他项目目录，不要引入独立 API 服务、worker、MCP server 或 monorepo 结构。

当前产品已开放页面化本地管理员、加密凭据、OpenAI/DeepSeek/Qwen/GLM 连接、项目级模型路由、GitHub 多仓库只读扫描、自动抽取、统一向量索引、语义检索和引用式 RAG。产品说明必须继续区分真实可执行能力与仍未实现的能力；未通过实现与运行验收的内容不得在 README、产品文案或 UI 中宣称可用。
...
每次有行为变化都要运行相关的聚焦检查。V2.0 发布前必须运行 README 中列出的项目级命令、真实 PostgreSQL AI 闭环、迁移状态、Docker Compose 健康/API smoke 和认证后页面 smoke；不要将只通过静态检查称作完成。

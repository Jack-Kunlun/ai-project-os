...
function RepositorySection({ projectId, repositories, credentialSuffix, onReload }: { projectId: string; repositories: RepositoryLink[]; credentialSuffix: string | null; onReload: () => Promise<void> }) {
  ...
  async function runTask(path: string, body: object, key: string) { setActiveTask(key); setMessage(null); try { const response = await fetch(`/api/projects/${projectId}/repositories/${path}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...body, clientKey: crypto.randomUUID() }) }); if (!response.ok) throw new Error(await readError(response, "仓库任务失败")); const payload = await response.json() as { job?: Job }; const result = payload.job?.result as GitHubJobResult | null | undefined; const hasWarning = Boolean(result?.warning || result?.warnings?.length); if (payload.job?.status === "unknown") setMessage("任务结果未知，未确认是否发布；请在任务详情中协调确认，系统不会自动重试。 "); else if (payload.job?.kind === "githubProjectSync" && result?.syncRunId) setMessage(`项目同步已结束：${result.status ?? payload.job.status}${hasWarning ? "（含未完成目标，请查看详情）" : ""}。可在下方任务列表打开同步详情。`); else if (payload.job?.status === "failed") { const failure = projectJobFailurePresentation(payload.job.failureCode); setMessage(`任务未完成：${failure.summary} ${failure.action}${failure.code ? `（错误代码：${failure.code}）` : ""}`); } else if (payload.job?.status === "succeeded" && hasWarning) setMessage("任务已完成，但部分可选仓库未完成；已完成内容按原子规则发布。 "); else if (payload.job?.status === "succeeded") setMessage("任务已完成，结果已原子发布"); else setMessage("任务仍在处理中，请查看任务状态"); await onReload(); } catch (taskError) { setMessage(taskError instanceof Error ? taskError.message : "仓库任务失败"); } finally { setActiveTask(null); } }

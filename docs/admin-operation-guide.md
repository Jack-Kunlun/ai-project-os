...
管理员配置并验证平台托管模型，并为视觉、抽取、向量和生成能力维护默认路由。DeepSeek 可用于生成能力，GLM 可按已验证能力用于向量能力；不要把某个 provider 的存在写成永久免费的承诺。普通用户不能配置个人模型，只能消费平台额度；只有有效会员可以维护个人模型连接，会员到期或撤销后不能继续测试、启用或调用。

...
当前版本状态：`CONTROLLED_PRERELEASE` · `0.6.0-dev.8（受控迁移预发布）`
...
正式稳定版基线：无。当前生产源版本为 `v0.6.0-dev.7`，当前受控迁移候选为 `v0.6.0-dev.8`；两者仍是预发布，不是稳定版或 GitHub Latest。只有精确的目标和源标签可进入当前生产通道。
...
0.6.0-dev.8 将一级导航统一为 Dashboard、个人工作台、我的项目、我的团队；个人工作台按总览、知识库、配置组织，额度详情独立成页。Git/MCP 连接必须先测试成功再保存，连接示例通过 tooltip 提供，高级网络安全设置默认展开；项目卡片统一为单一入口，Dashboard 增加个人内容并统一卡片按钮底部对齐。
...
- 当前发布目标为 `0.6.0-dev.8`，只允许从生产实际运行的 `v0.6.0-dev.7` 进入 107→116 迁移通道；这些版本都不是稳定版或 GitHub Latest。平台管理员只负责平台运营和安全治理，不进入项目、团队或用户工作区；普通用户负责自己的项目和个人连接。
...
面向局域网外提供服务前，应按[部署安全基线](docs/deployment-security.md)配置 HTTPS、入口限流和可信反向代理，并设置 `AI_PROJECT_OS_SECURE_COOKIES=true` 与实际 HTTPS `AI_PROJECT_OS_PUBLIC_ORIGIN`。仓库提供的 Nginx 示例必须替换域名与证书路径并通过 `nginx -t` 后才能启用。`v0.6.0-dev.8` 必须从已验证的 `.7` 基线通过 stopped-writer 迁移通道验证后才可进入生产。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖当前 `0.6.0-dev.8` 受控迁移预发布的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准；`v0.6.0-dev.7` 作为唯一允许的生产源版本保留。

...
当前代码版本状态：`CONTROLLED_PRERELEASE` · `0.6.0-dev.13（受控发布候选）`
...
正式稳定版基线：无。`v0.6.0-dev.11` 已从 `.10` 按固定的 116→117 迁移路径部署并验收；本候选从 `.12` 通过无迁移的 **Deploy application** 通道发布。实际生产版本以公网 `/api/health` 为准；这些版本仍是预发布，不是稳定版或 GitHub Latest。
...
- 当前发布目标为 `0.6.0-dev.13`，从生产实际运行的 `v0.6.0-dev.12` 走无迁移应用发布通道；这些版本都不是稳定版或 GitHub Latest。平台管理员只负责平台运营和安全治理，不进入项目、团队或用户工作区；普通用户负责自己的项目和个人连接。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖当前 `0.6.0-dev.13` 受控发布候选的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准；本次无迁移应用发布以线上健康接口核实的 `v0.6.0-dev.12` 为源。

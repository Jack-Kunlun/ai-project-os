...
AI Project OS V2.2.1 是一套可在本机部署的项目长期记忆与只读项目智能工作台。它把人工资料和多个 GitHub 仓库整理成可追溯记忆，并通过 Dashboard 汇总配置进度、项目状态、最近任务和下一步操作，提供页面化的模型配置、自动抽取、向量索引、语义检索、带引用 RAG、项目状态简报和受约束的只读智能体。
...
- 已有 V1 CLI 和历史运行合同继续保留用于兼容与审计，但 V2.2.1 的常规配置和操作入口是网页。
...
V1 CLI 手册和历史合同保留在 `docs/`，仅用于兼容、审计和理解旧数据结构。它们不覆盖本 README 描述的当前 V2.2.1 页面能力。

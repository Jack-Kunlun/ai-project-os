...
export function LoginForm({
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_8%_5%,rgba(224,231,255,.72),transparent_24%),radial-gradient(circle_at_92%_94%,rgba(237,233,254,.68),transparent_25%),#f7f9fd] px-4 py-7 text-slate-950 sm:px-7 sm:py-9">
      <div className="mx-auto grid w-full max-w-[1476px] overflow-hidden rounded-[38px] border border-white/90 bg-white shadow-[0_24px_70px_rgba(15,23,42,.13)] lg:min-h-[808px] lg:grid-cols-2">
        <section className="relative order-2 overflow-hidden bg-[radial-gradient(circle_at_91%_0%,rgba(91,33,182,.56),transparent_34%),linear-gradient(145deg,#060b19_0%,#070d1d_60%,#11102c_100%)] px-8 py-10 text-white sm:px-12 sm:py-12 lg:order-1 lg:px-12 lg:py-[52px] lg:pl-[55px]">
          ...
          <div className="relative flex min-h-full flex-col">
            <div className="flex items-center gap-4">
              <span className="inline-flex h-[50px] w-[50px] items-center justify-center rounded-2xl bg-white text-[15px] font-black tracking-tight text-slate-950 shadow-lg">OS</span>
              <span className="text-[16px] font-semibold tracking-[-0.01em] text-slate-100">AI Project OS</span>
            </div>

            <div className="mt-14 lg:mt-[58px]">
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-indigo-400">AI Project OS</p>
              <h1 className="mt-5 max-w-[620px] text-[38px] font-semibold leading-[1.23] tracking-[-0.045em] sm:text-[48px] lg:text-[50px]">让项目知识可追溯，<br />让每一次行动都有边界。</h1>
              <p className="mt-6 max-w-[620px] text-[15px] leading-8 text-slate-300">把项目资料、代码来源、当前状态和团队协作放在同一个受控空间中。<br className="hidden xl:block" />系统保留原始证据与引用链，帮助团队从可靠资料出发理解项目。</p>
            </div>
            ...
            <div className="mt-auto flex items-start gap-4 pt-8 text-[12px] leading-6 text-slate-400">
        ...
        <section className="order-1 flex items-center bg-[radial-gradient(circle_at_75%_15%,rgba(238,242,255,.8),transparent_36%),#fff] px-7 py-12 sm:px-14 lg:order-2 lg:px-[86px] lg:py-[70px]">
          <form onSubmit={submit} className="mx-auto w-full max-w-[580px]">
            <p className="text-xs font-bold uppercase tracking-[0.27em] text-indigo-600">Secure workspace</p>
            <h2 className="mt-4 text-[34px] font-semibold tracking-[-0.04em] sm:text-[38px]">登录 AI Project OS</h2>
            <p className="mt-3 text-[15px] leading-7 text-slate-500">使用账号登录你的工作区，继续处理项目资料与治理任务。</p>

            <label className="mt-9 block text-sm font-semibold" htmlFor="login-username">用户名</label>
            <div className="relative mt-3">
              <span className="pointer-events-none absolute inset-y-0 left-5 flex items-center text-slate-400"><UserIcon /></span>
              <input id="login-username" value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" minLength={3} maxLength={64} required placeholder="输入登录名" className="h-16 w-full rounded-xl border border-slate-200 bg-slate-50/80 pl-[60px] pr-5 text-[16px] outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100" />
            </div>

            <label className="mt-7 block text-sm font-semibold" htmlFor="login-password">密码</label>
            <div className="relative mt-3">
              <span className="pointer-events-none absolute inset-y-0 left-5 flex items-center text-slate-400"><LockIcon /></span>
              <input id="login-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required placeholder="输入密码" className="h-16 w-full rounded-xl border border-slate-200 bg-slate-50/80 pl-[60px] pr-5 text-[16px] outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100" />
            </div>
            ...
            <button type="submit" disabled={pending} className="mt-8 h-[58px] w-full rounded-xl bg-[linear-gradient(90deg,#4f35ff,#4a2df3)] px-4 text-[16px] font-semibold tracking-[0.22em] text-white shadow-lg shadow-indigo-500/20 transition hover:brightness-110 disabled:opacity-50">{pending ? "登录中…" : "登 录"}</button>

            <div className="my-7 flex items-center gap-5 text-sm text-slate-400"><span className="h-px flex-1 bg-slate-200" /><span>其他登录方式</span><span className="h-px flex-1 bg-slate-200" /></div>

            {githubLoginAvailable ? (
              <a href={githubHref} className="flex h-[58px] w-full items-center justify-center gap-4 rounded-xl border border-slate-300 bg-white text-[16px] font-semibold text-slate-900 transition hover:border-indigo-300 hover:bg-indigo-50"><GitHubIcon />使用 GitHub 登录</a>
            ) : (
              <div>
                <button type="button" disabled className="flex h-[58px] w-full items-center justify-center gap-4 rounded-xl border border-slate-200 bg-slate-50 text-[16px] font-semibold text-slate-500"><GitHubIcon />使用 GitHub 登录</button>
      ...
      <footer className="mx-auto mt-7 flex max-w-[1476px] items-center justify-center gap-5 text-xs text-slate-400 sm:gap-8">
...
function Feature({ icon, iconTone, title, children }: { icon: ReactNode; iconTone: string; title: string; children: ReactNode }) {
  return <div className="flex min-h-[116px] items-start gap-4 rounded-2xl border border-white/15 bg-white/[.045] p-4"><span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${iconTone}`}>{icon}</span><div><p className="text-[16px] font-semibold text-white">{title}</p><p className="mt-1.5 text-[13px] leading-6 text-slate-400">{children}</p></div></div>;
}

...
test("0.6 gateway accepts one exact migration grammar and rejects injection", async () => {
  ...
  assert.ok(gateway.includes("^deploy-v06\\ (v0\\.5\\.0-dev\\.(1|4))\\ (v0\\.6\\.0-dev\\.6)\\ ([0-9a-f]{40})\\ (CONFIRM_V06_MIGRATION_V1)$"));
  assert.match(gateway, /exec sudo -n \/usr\/local\/sbin\/ai-project-os-v06-deploy/u);

  for (const command of [
    `deploy-v06 v0.5.0-dev.2 v0.6.0-dev.6 ${"a".repeat(40)} CONFIRM_V06_MIGRATION_V1`,
    `deploy-v06 v0.5.0-dev.4 v0.6.0-dev.2 ${"a".repeat(40)} CONFIRM_V06_MIGRATION_V1`,
    `deploy-v06 v0.5.0-dev.4 v0.6.0-dev.6 ${"a".repeat(40)} CONFIRM_V06_MIGRATION_V1; id`,
  ]) {
...
test("0.6 deploy verifies source, stopped writers, backup, migration, and health", async () => {
  ...
  assert.match(deploy, /RELEASE_TAG" != v0\.6\.0-dev\.6/u);

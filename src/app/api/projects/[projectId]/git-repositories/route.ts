...
import { connectProjectGitRepository, listProjectGitRepositories } from "@/lib/git";
...
export async function GET(request: Request, context: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    return NextResponse.json({ repositories: await listProjectGitRepositories(await projectId(context.params)) });
  } catch (error) {

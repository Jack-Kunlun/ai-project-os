...
import { confirmProjectMcpConnectionDelegationOwner } from "@/lib/project-mcp-connection-delegation-service";
...
export async function POST(request: Request, context: { params: Promise<{ projectId: string; delegationId: string }> }) {
  try {
    ...
    const result = await confirmProjectMcpConnectionDelegationOwner(

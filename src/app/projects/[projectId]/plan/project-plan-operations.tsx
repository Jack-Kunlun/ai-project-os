...
export function ProjectOperationsSummary({ health, impacts, workItems, canEdit, pending, request }: {
  ...
  const healthLabel = health.status === "atRisk" ? "需要立即处理" : health.status === "attention" ? "存在待处理事项" : health.status === "empty" ? "尚未建立工作项" : "运行正常";
  const tone = health.status === "atRisk" ? "border-rose-200 bg-rose-50 text-rose-900" : health.status === "attention" ? "border-amber-200 bg-amber-50 text-amber-950" : health.status === "empty" ? "border-slate-200 bg-slate-50 text-slate-800" : "border-emerald-200 bg-emerald-50 text-emerald-900";

  return <>
    <section className={`mt-8 rounded-3xl border p-6 ${tone}`}>
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] opacity-70">Project health</p><h2 className="mt-2 text-2xl font-semibold">{healthLabel}</h2><p className="mt-2 max-w-3xl text-sm leading-6 opacity-80">健康状态由期限、受阻、依赖、负责人、验收标准、证据、待采纳建议、仓库变更和动作审批确定性计算，不调用模型。尚未建立工作项时不会推断为运行正常。</p></div>
...
export function WorkItemOperations({ item, objectives, members, evidenceLinks, evidenceCandidates, canEdit, pending, request }: {
  ...
  return <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold text-slate-700">负责人、验收与证据</p><p className="mt-1 text-[12px] text-slate-400">开始前需要负责人和验收标准；完成前还需要至少一条活动证据。</p></div>{terminal ? <span className="rounded-full bg-slate-200 px-3 py-1 text-[12px] font-semibold text-slate-500">终态只读</span> : null}</div>
...
function HealthMetric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl border border-current/10 bg-white/60 px-3 py-3"><p className="text-xl font-semibold">{value}</p><p className="mt-1 text-[12px] opacity-70">{label}</p></div>;
}

...
当前产品已开放页面化多用户/RBAC、邀请与 OIDC、加密凭据、OpenAI/DeepSeek/Qwen/GLM、项目 AI 路由、文件与视觉识别、多 Git 代码仓库、网页来源、本地文件夹、自动抽取、统一向量索引、语义检索、引用式 RAG、持久化自动化、受控动作与审批、记忆质量、项目简报和受约束的只读项目智能体。产品说明必须区分真实能力与未实现能力；未通过实现与运行验收的内容不得在 README、产品文案或 UI 中宣称可用。
...
- Action Engine 只能调用显式白名单中的内置能力。动作必须绑定项目、规范化输入、输入指纹、能力风险和策略快照；审批记录、策略修订和审计记录不可变。项目 Owner 才能修改策略或审批，租约过期必须失败关闭且不得自动重试外部读取。
...
每次行为变化运行与风险相称的聚焦检查。V3.1.0 发布验收必须包含：Prisma 生成与校验、完整迁移链、类型检查、Lint、测试、生产构建、真实 PostgreSQL V3 与 Action Engine 闭环、Docker Compose `postgres/migrate/app/worker` 状态、认证后页面 smoke，以及按钮文字居中和共享 Header 一致性。真实外部模型、Git 或 OIDC 服务未配置时，必须明确说明对应连接未做现场验证，不得用静态检查替代。

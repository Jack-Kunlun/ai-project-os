...
AI Project OS V4.1.0 是一套可本地部署的项目长期记忆、只读项目智能、受控动作与证据驱动运营工作台。它把人工资料、文件、网页、多个 Git 服务中的代码仓库和经人工纳入的 MCP 只读结果整理为有来源、有版本、有审核状态的项目证据，并提供语义检索、引用式 RAG、项目状态简报、受约束的只读智能体、持久化自动化、动作审批、负责人/期限/验收/证据闭环、确定性项目健康、不可变审计、记忆质量治理和团队权限。
...
12. 在“项目计划”中为工作项设置负责人、期限和验收标准，关联已确认事实、活动来源或仓库同步证据，再由人推进状态。
13. 人工核对仓库变化信号并关联到相关工作项，或明确忽略；需要定期提醒时创建“项目计划健康提醒”自动化。
14. 需要协作时，在“团队”中创建成员、邀请链接或 OIDC 身份源。

## 当前能力

| 能力 | V4.1.0 行为 |
| --- | --- |
| Dashboard | 汇总跨项目指标、配置就绪度、计划风险、运营提醒、推荐下一步和最近任务；项目管理保持为独立顶部入口 |
...
- V4.1.0 当前提供一个由旧数据迁移得到的默认工作区；数据模型支持多个工作区，但页面尚未提供工作区创建和切换。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖 V4.1.0 的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准。

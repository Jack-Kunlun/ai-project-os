...
第一次打开本地地址进入初始化页面。平台管理员用户名为 3–64 位，密码为 12–128 位且同时包含字母和数字。随后由平台管理员创建一个独立的普通用户账号作为默认工作区 Owner。平台管理员只进入 `/admin`，普通用户 Owner 只进入用户工作台、项目和团队页面；两种身份不共享项目、团队或个人连接权限。

初始化账号、初始密码和任何恢复材料不得写进 Git、Issue、截图或日志。初始化完成后，普通用户通过团队邀请或受控企业登录进入；平台管理员继续使用独立管理工作台。

...
AI Project OS V5.1.2 是一套可本地部署的项目长期记忆、时态项目世界模型、只读项目智能、受控动作与证据驱动运营工作台。它把人工资料、文件、网页、多个 Git 服务中的代码仓库和经人工纳入的 MCP 只读结果整理为有来源、有版本、有审核状态的项目证据，再把当前有效事实、人工关系、替代链、冲突和项目计划合成为可复现的当前状态，并提供语义检索、引用式 RAG、项目状态简报、受约束的只读智能体、持久化自动化、动作审批、不可变审计、记忆质量治理和团队权限。
...
- 未发布候选状态：[docs/releases/next.md](docs/releases/next.md)。当前候选为 V5.1.2。
- V5.1.2 候选说明：[docs/releases/v5.1.2.md](docs/releases/v5.1.2.md)。

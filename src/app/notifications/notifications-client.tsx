...
export function notificationDetailHref(href: string, filter: NotificationFilter, cursor: string | null, focus: string | null): string | null {
  try {
    ...
    const supportsReturnContext = /^\/projects\/[^/]+\/(?:actions|jobs\/[^/]+)$/u.test(target.pathname);
...
export function NotificationsClient({ username, isSystemAdmin = false }: { username: string; isSystemAdmin?: boolean }) {
  ...
  const reload = useCallback(async (options: Readonly<{ append?: boolean; cursorOverride?: string | null }> = {}) => {
    ...
    try {
      ...
      const payload = await response.json() as { notifications: Notification[]; unreadCount: number; nextCursor: string | null };
  ...
  async function open(item: Notification): Promise<void> {
    ...
    try {
      ...
      const actionHref = typeof opened.actionHref === "string" ? opened.actionHref : item.actionHref;
      if (actionHref !== null) {
        const detailCursor = notificationCursorForId(notificationCursorByIdRef.current, item.id, cursor);
        const destination = notificationDetailHref(actionHref, filter, detailCursor, item.id);
  ...
  const emptyMessage = filter === "unread" ? "当前没有未读通知。切换到“全部”可查看历史记录。" : filter === "system" ? "当前没有系统通知。" : "还没有活动记录。耗时任务完成、失败或需要人工确认后会出现在这里。";

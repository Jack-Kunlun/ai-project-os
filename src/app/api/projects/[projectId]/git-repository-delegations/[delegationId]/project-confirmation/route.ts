...
import { confirmProjectGitRepositoryDelegationProject } from "@/lib/project-git-repository-delegation-service";
...
export async function POST(request: Request, context: { params: Promise<{ projectId: string; delegationId: string }> }) {
  try {
    ...
    const result = await confirmProjectGitRepositoryDelegationProject(

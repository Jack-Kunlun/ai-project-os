...
AI Project OS V3.1.0 是一套可本地部署的项目长期记忆、只读项目智能与受控动作工作台。它把人工资料、文件、网页和多个 Git 服务中的代码仓库整理为有来源、有版本、有审核状态的项目记忆，并提供语义检索、引用式 RAG、项目状态简报、受约束的只读智能体、持久化自动化、动作审批、不可变审计、记忆质量治理和团队权限。
...
9. 在项目“动作与审批”中选择动作策略，核对待审批动作和执行审计。
10. 需要协作时，在“团队”中创建成员、邀请链接或 OIDC 身份源。
...
| 动作与审批 | 项目级策略控制自动执行、每次审批或禁止执行；当前只开放仓库同步、网页刷新和记忆质量检查，动作绑定输入指纹与策略快照并保留不可变审批和审计记录 |
...
- V3.1.0 当前提供一个由旧数据迁移得到的默认工作区；数据模型支持多个工作区，但页面尚未提供工作区创建和切换。
...
- 动作中心不是通用 Agent 工具平台。当前动作只接受空参数并调用三个内置能力；不接受模型生成的工具调用，不开放 MCP、Shell、代码写入、分支/PR、合并、部署或任何外部写操作。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖 V3.1.0 的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准。

...
状态：冻结版本 `ai-memory-eval-v1.0`。冻结的是合成评测合同与 gold 基线，不是实际 Recall、模型质量、GitHub/E2E 或 V1 可用性通过；这些能力仍未验证。
...
这是一份可提交、可复核、与模型供应商无关的合成评测基线。它用于约束后续受控模型处理、检索、RAG、连接器和只读代理的输入输出语义；当前 V0 不执行这些能力，也不在本基线中调用真实模型、网络、GitHub、数据库或外部评测服务。

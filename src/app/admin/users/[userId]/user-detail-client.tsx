...
export function AdminUserDetailClient({ userId }: { userId: string }) {
  ...
  if (loading && detail === null) return <div className="mx-auto max-w-6xl px-6 py-12 text-sm text-slate-500">读取中…</div>;
  if (error || summary === null || detail === null) return <div className="mx-auto max-w-6xl px-6 py-12"><p role="alert" className="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">{error ?? "普通用户不存在"}</p><Link href="/admin/users" className="mt-5 inline-flex text-sm font-semibold text-indigo-700">返回用户运营</Link></div>;

  return <div className="mx-auto max-w-6xl px-6 pb-16 pt-8 sm:px-10 lg:px-12">
    <div className="mb-5"><Link href="/admin/users" className="text-xs font-semibold text-indigo-700">← 返回用户运营</Link></div>
    <header className="rounded-[2rem] bg-slate-950 px-7 py-8 text-white shadow-2xl shadow-slate-950/15 sm:px-10 sm:py-10">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-indigo-300">User operations</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">{summary.user.displayName ?? summary.user.username}</h1><p className="mt-2 text-sm text-slate-300">{summary.user.username}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${summary.user.state === "enabled" ? "bg-emerald-400/15 text-emerald-200" : "bg-rose-400/15 text-rose-200"}`}>{summary.user.state === "enabled" ? "账号已启用" : "账号已停用"}</span></div>
      <p className="mt-5 text-xs text-slate-400">统一处理账号状态、会员和额度；每个危险变更都会先预览并在执行前重新校验。</p>
    </header>

  ...
  return (
    ...
    >
      ...
      {archived ? (
        ...
        <div className="mt-5">
          <div className="flex items-center justify-between text-[12px] text-slate-400"><span>资料与记忆进度</span><span>{progress}%</span></div>
...
function CreateProjectDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/45 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !pending) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="create-project-title" className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg flex-col overflow-hidden rounded-t-[2rem] bg-white shadow-2xl sm:rounded-[2rem]">
      <header className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-100 px-7 py-5 sm:px-8">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-600">New project</p><h2 id="create-project-title" className="mt-2 text-2xl font-semibold">创建项目空间</h2></div>
        <button type="button" onClick={onClose} disabled={pending} className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200" aria-label="关闭">×</button>
      </header>
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-7 py-5 sm:px-8">
        <p className="text-sm leading-6 text-slate-500">项目会隔离自己的资料、仓库和智能记忆；模型能力由平台默认模型或个人双确认委托提供。</p>
        <form id="projects-create-form" onSubmit={submit} className="mt-5">
          <label htmlFor="projects-project-name" className="text-sm font-semibold text-slate-700">项目名称</label>
          <input id="projects-project-name" autoFocus value={name} onChange={(event) => setName(event.target.value)} maxLength={120} required placeholder="例如：AI Project OS" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" />
          <label htmlFor="projects-project-description" className="mt-5 block text-sm font-semibold text-slate-700">项目描述 <span className="font-normal text-slate-400">（可选）</span></label>
          <textarea id="projects-project-description" value={description} onChange={(event) => setDescription(event.target.value)} maxLength={2000} rows={4} placeholder="这个项目在解决什么问题？当前最重要的目标是什么？" className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" />
      ...
      <footer className="flex shrink-0 justify-end gap-3 border-t border-slate-100 bg-white px-7 py-5 sm:px-8">
        <button type="button" onClick={onClose} disabled={pending} className="rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600">取消</button>
        <button type="submit" form="projects-create-form" disabled={pending || !name.trim()} className="rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{pending ? "创建中…" : "创建项目"}</button>
      </footer>
    </section>
  </div>;
}

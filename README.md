...
当前代码版本状态：`CONTROLLED_PRERELEASE` · `0.6.0-dev.12（受控发布候选）`
...
正式稳定版基线：无。`v0.6.0-dev.11` 已从 `.10` 按固定的 116→117 迁移路径部署并验收；本候选从 `.11` 通过无迁移的 **Deploy application** 通道发布。实际生产版本以公网 `/api/health` 为准；这些版本仍是预发布，不是稳定版或 GitHub Latest。
...
0.6.0-dev.8 曾将一级导航统一为 Dashboard、个人工作台、我的项目、我的团队，并加入连接测试门禁与额度页。当前 `.11` 将“我的项目”纳入个人工作台子导航，项目成员在项目内管理，团队邀请和账号仍留在团队空间；个人知识支持图片、扫描 PDF 和混合 PDF 的可选视觉提取、审核式知识图谱与显式标记的个人通用记忆。Git/MCP 已验证连接可重新测试后修改地址与认证。
...
| 能力 | 0.6.0-dev.12 受控发布候选行为 |
| --- | --- |
| Dashboard | 汇总跨项目世界状态、配置就绪度、计划风险、运营提醒、推荐下一步和最近任务；项目入口位于个人工作台内 |
...
- 当前发布目标为 `0.6.0-dev.12`，从生产实际运行的 `v0.6.0-dev.11` 走无迁移应用发布通道；这些版本都不是稳定版或 GitHub Latest。平台管理员只负责平台运营和安全治理，不进入项目、团队或用户工作区；普通用户负责自己的项目和个人连接。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖当前 `0.6.0-dev.12` 受控发布候选的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准；本次无迁移应用发布以线上健康接口核实的 `v0.6.0-dev.11` 为源。

...
export default async function AdminGuidePage() {
  ...
  return <div className="w-full px-4 pb-12 pt-5 sm:px-5 lg:px-6">
    ...
    <div className="mt-6 grid gap-4 lg:grid-cols-2">
      ...
      <GuideCard title="2. 新增供应商：先测试后保存" summary="新增连接必须先用真实凭据测试，测试证据通过后才能保存。" steps={["在平台模型页点击“新增供应商”，填写固定供应商、端点、模型和 API Key。", "点击“测试连接”，等待指定能力返回成功证据。", "确认测试仍在有效期内后保存连接；API Key 不会写入浏览器存储或响应。"]} success="测试状态为成功且保存按钮可用，连接出现在已配置连接列表。" blockers="探测预算未启用/不足、端点或凭据错误、测试证据过期；修正表单后重新测试。" href="/settings" linkLabel="打开平台模型" />

...
0.2.x 当前仅表示进入产品改造开发线，不代表目标能力已经交付。会员个人模型、Git/MCP 用户私有所有权和平台默认模型路由仍处于开发/计划阶段，本版本不宣称这些能力已经可用。
...
3. 进入项目仓库页查看已有安全摘要；旧版项目 Git 连接、首次关联和同步入口已冻结。个人 Git 连接与项目委托仍在改造中，当前不会在管理员页面收集用户凭据。
4. MCP 连接与项目运行时入口在迁移期间冻结；个人连接所有权、管理员安全审查和项目授权页面仍在改造中，当前不会在管理员页面收集 Bearer Token。
...
| 多 Git 连接 | 个人 Git 连接服务端边界正在建设；旧版项目/管理员连接入口已冻结，当前页面不收集或提交 Token、密码、私钥、CA 或 known_hosts |
| 受控 MCP 连接与结果纳入 | 个人连接所有权、工具发现、管理员安全审查、项目授权和结果纳入仍在改造；旧版管理员配置与项目运行时入口已冻结 |
| 多仓库记忆 | 已关联仓库的历史资料保留安全边界；项目仓库新增、同步和停用入口在迁移期间冻结，个人 Git 连接与项目委托属于 planned 能力 |
| GitHub 扩展资料 | 既有 GitHub 专用项目连接、CLI 和同步入口在迁移期间冻结；个人连接委托属于 planned 能力 |
...
| 动作与审批 | 项目级策略控制自动执行、每次审批或禁止执行；仓库同步与 MCP 外发入口在迁移期间冻结，网页刷新和记忆质量检查仍按项目策略运行 |
...
- 当前 `0.2.0-dev.1` 开发线承接 `v0.1.0-dev.1` 的默认工作区；数据模型支持多个工作区，但页面尚未提供工作区创建和切换。0.2.x 目标中的会员个人模型、Git/MCP 用户私有所有权和平台默认模型路由仍未实现；旧版项目 GitHub、管理员连接和 MCP attestation 入口保持冻结。
...
- 动作中心不是通用 Agent 工具平台。三个内置能力仍使用固定输入；MCP 旧版管理员认证、Owner 授权和远程调用入口在迁移期间冻结，未来开放时仍不得接受模型生成或批准的工具调用。

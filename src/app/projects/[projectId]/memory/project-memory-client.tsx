...
function MemoryIndexConsentCheck({
  ...
  const details = plan === null
    ? "请先读取当前索引计划。"
    : `${modeLabel}构建将使用 ${plan.providerName ?? plan.providerKind ?? "当前路由"} 的 ${plan.modelId ?? "受限模型"}（${plan.dimensions ?? "未知"} 维），待生成 ${plan.generateCount} 条、复用 ${plan.reuseCount} 条，预计 ${plan.estimatedProviderCalls} 次向量请求。所有待生成片段会发送给该供应商；复用片段不会再次外发。`;
                ...
                <article key={candidate.id} className={`min-w-0 max-w-full rounded-2xl border bg-white p-5 transition ${activeSelectedCandidates.includes(candidate.id) ? "border-indigo-300 ring-1 ring-indigo-100" : "border-slate-200"}`}>
                  <label className="flex min-w-0 cursor-pointer items-start gap-3">
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex min-w-0 max-w-full flex-wrap items-center gap-2"><span className="shrink-0 rounded-full bg-indigo-50 px-2.5 py-1 text-[11px] font-semibold text-indigo-700">{candidate.projectItem.type}</span><span className="min-w-0 text-[11px] text-slate-400 [overflow-wrap:anywhere]">{candidate.providerConnection?.name ?? candidate.providerConnection?.kind ?? "个人连接"} · {candidate.modelId ?? "模型信息受限"}</span></span>

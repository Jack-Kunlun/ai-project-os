...
test("production workflow is manual, serialized, least-privilege, and tag-CI-gated", async () => {
  ...
  assert.match(workflow, /Create verified offsite backup and deploy migration/u);
  ...
  assert.ok(
    workflow.indexOf("Sync GitHub OAuth configuration through restricted stdin") <
      workflow.indexOf("Create verified offsite backup and deploy migration through forced-command gateway"),
...
test("production workflow enables only the explicitly approved v0.6.0-dev.8 prerelease", async () => {
  const workflow = await readFile(workflowPath, "utf8");
...
test("forced-command gateway accepts only exact release tooling, preserve-data, or GitHub OAuth commands", async () => {
  ...
  assert.match(gateway, /v0\\\.5\\\.0-dev\\\.4/u);
  ...
  assert.match(gateway, /deploy-v06-next/u);
...
test("clean deploy is fail-closed, backup-first, and never migrates the old database", async () => {
  ...
  assert.match(deployment, /RELEASE_TAG.*v0\.5\.0-dev\.1/u);
...
test("backup quiesces writers, verifies encrypted COS objects, and deletes only marked local backups", async () => {
  ...
  assert.match(backup, /pre-deploy-to-v0\\\.5\\\.0-dev\\\.\(1\|2\|3\|4\)/u);
...
test("deploy pre-deploy backup is the quiesced artifact accepted by migration restore", async () => {
  ...
  assert.match(restore, /MIGRATION_SOURCE_VERSION=0\.4\.0-dev\.1/u);
...
test("backup publishes an atomic sanitized current record and verified history entry", async (context) => {
  ...
  const backupName = "20260902T032000Z-pre-deploy-to-v0.5.0-dev.1.Abc123";
...
readonly TARGET_TAG=v0.5.0-dev.1
...
test("production shell entrypoints pass bash syntax validation", () => {
  ...
  for (const scriptPath of [deploymentPath, v06DeploymentPath, v06NextDeploymentPath, cleanDeploymentPath, backupPath, gatewayPath, releaseToolingInstallerPath, githubOAuthConfiguratorPath, installerPath, backupInstallerPath, ...additionalScripts]) {

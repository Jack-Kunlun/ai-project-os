...
import { buildProjectHref, parseProjectHref, parseProjectPageState } from "@/lib/project-navigation";
...
export function ProjectJobDetailClient({ username, isSystemAdmin }: { username: string; isSystemAdmin: boolean }) {
  ...
  const returnRoute = safeReturnTo === null ? null : parseProjectHref(projectId, safeReturnTo)?.route ?? null;
  const governanceBackHref = navigation.from === "governance" ? buildProjectHref(projectId, "governance", {
    status: navigation.status,
    kind: navigation.kind,
    search: navigation.search,
    cursor: navigation.cursor,
    focus: "task-runs",
    from: returnRoute === "overview" ? "overview" : null,
    returnTo: safeReturnTo,
  }) : null;
  const backHref = governanceBackHref ?? safeReturnTo ?? (fromNotifications ? `/notifications?view=${notificationFilter}${notificationPageCursor ? `&cursor=${encodeURIComponent(notificationPageCursor)}` : ""}${notificationFocusId ? `&focus=${encodeURIComponent(notificationFocusId)}` : ""}` : job ? parentRoute(projectId, job.kind) : `/projects/${projectId}/governance#task-runs`);

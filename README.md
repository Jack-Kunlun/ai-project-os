...
当前版本状态：`CONTROLLED_PRERELEASE` · `0.6.0-dev.11（受控迁移候选）`
...
正式稳定版基线：无。当前生产源版本为 `v0.6.0-dev.10`，当前受控迁移候选为 `v0.6.0-dev.11`；两者仍是预发布，不是稳定版或 GitHub Latest。本次发布只允许固定的 `.10`→`.11`、116→117 迁移路径。
...
0.6.0-dev.8 曾将一级导航统一为 Dashboard、个人工作台、我的项目、我的团队，并加入连接测试门禁与额度页。当前 `.11` 候选将“我的项目”纳入个人工作台子导航，项目成员在项目内管理，团队邀请和账号仍留在团队空间；个人知识支持图片、扫描 PDF 和混合 PDF 的可选视觉提取、审核式知识图谱与显式标记的个人通用记忆。Git/MCP 已验证连接可重新测试后修改地址与认证。
...
- 当前发布目标为 `0.6.0-dev.11`，只允许从生产实际运行的 `v0.6.0-dev.10` 进入 116→117 迁移通道；这些版本都不是稳定版或 GitHub Latest。平台管理员只负责平台运营和安全治理，不进入项目、团队或用户工作区；普通用户负责自己的项目和个人连接。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖当前 `0.6.0-dev.11` 受控迁移候选的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准；本次迁移的唯一允许生产源为 `v0.6.0-dev.10`。

...
import { runV06DefaultMemoryUpgradePreflight } from "./production-v06-default-memory-upgrade-preflight";
...
export async function runAppUpgradePreflight(phase: string, targetTag: string, sourceTag: string, databaseUrl: string) {
  ...
  if (!tagPattern.test(sourceTag)) throw new Error("APP_PREFLIGHT_SOURCE_INVALID");
  const sourceSequence = Number(sourceTag.slice("v0.6.0-dev.".length));
  if (!Number.isSafeInteger(sourceSequence) || sourceSequence < 8) throw new Error("APP_PREFLIGHT_SOURCE_INVALID");
  const migrationCount = sourceSequence >= 11 ? 117 : 116;
  const migrations = (await readdir(resolve(process.cwd(), "prisma/migrations")))
    .filter((name) => /^\d{14}_/u.test(name));
  if (migrations.length !== migrationCount) throw new Error("APP_PREFLIGHT_MIGRATION_MANIFEST_INVALID");
  if (migrationCount === 117) await runV06DefaultMemoryUpgradePreflight("post-migration", databaseUrl);
  else await runV06NextUpgradePreflight("post-migration", databaseUrl);
  if (phase === "post-stop") {
  ...
  return { ok: true, kind: "app-upgrade-preflight", phase, targetTag, sourceTag, migrationCount,
...
if (process.argv[1]?.endsWith("production-app-upgrade-preflight.ts")) {
  const [phase = "", targetTag = "", sourceTag = ""] = process.argv.slice(2);
  ...
  if (!databaseUrl) {
    ...
    runAppUpgradePreflight(phase, targetTag, sourceTag, databaseUrl).then(

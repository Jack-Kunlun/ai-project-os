...
export function MembershipsClient({ username }: { username: string }) {
  ...
  return <main className="min-h-screen bg-[#f5f7fb] text-slate-950"><AppHeader username={username} active="profile" /><div className="mx-auto max-w-6xl px-6 py-9 sm:px-10 lg:px-12"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-600">Membership operations</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.04em]">会员资格管理</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500">仅人工授予、延期或撤销会员资格，不会修改用户或工作区角色。</p></div><Link href="/profile" className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600">返回个人中心</Link></div><div className="mt-7 flex gap-3"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索用户名、邮箱或显示名称" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100" /><button type="button" onClick={() => void load()} className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white">搜索</button></div>{error ? <div role="alert" className="mt-5 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">{error}</div> : null}<section className="mt-6 space-y-3">{loading ? <div className="rounded-3xl border border-slate-200 bg-white p-10 text-sm text-slate-500">读取中…</div> : items.length === 0 ? <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">没有匹配的用户。</div> : items.map((item) => <MembershipCard key={item.id} item={item} onChanged={() => void load()} />)}</section></div></main>;
}

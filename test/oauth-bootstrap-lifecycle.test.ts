...
function fakeOidcDb(bootstrap: Readonly<{ initialOwnerUserId: string | null; adminOnboardingCompletedAt: Date | null }>) {
...
test("OIDC start fails closed before provider lookup while first-owner bootstrap is pending", async () => {
  const store = fakeOidcDb({ initialOwnerUserId: null, adminOnboardingCompletedAt: null });

...
当前版本仅记录进入 0.2.x 产品改造开发线，不代表目标功能已经交付。生产部署入口继续静态禁用，当前版本不得进入生产 tag 通道。

### 计划中的改造（尚未交付）

- 重新定义普通用户、付费会员和系统管理员的模型权益与平台额度边界。
- 支持付费会员配置个人模型，并将 Git/MCP 连接归属到具体用户。
- 支持管理员维护平台托管模型与默认模型路由。

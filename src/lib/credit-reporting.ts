...
export async function getCreditReportInTransaction(userId: string, db: ReportDb, query: CreditReportQuery, now: Date): Promise<CreditReport> {
  ...
  const [settledReservations, pendingReservations, ledgerTotal, ledgerRows, membershipApplication] = await Promise.all([
    db.platformTokenReservation.findMany({ where: { ...reservationWhere, status: "settled", settledAt: { gte: query.window.from, lt: query.window.to } }, select: { settledAt: true, allocations: { select: { settledTokens: true } } } }),
  ...
  const daily = Object.freeze(query.window.days.map((date) => Object.freeze({ date, settledCredits: dailySettled.get(date)!, pendingCredits: dailyPending.get(date)! })));

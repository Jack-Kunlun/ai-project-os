    ...
    <div className="mx-auto max-w-6xl px-5 pb-16 pt-7 sm:px-8 lg:px-10">
      ...
      <section className="mt-6 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm sm:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="rounded-2xl bg-indigo-50 px-4 py-3 text-sm text-indigo-900 lg:max-w-xs">
            ...
            <p className="mt-1 text-xs leading-5">项目页已支持一次性手动只读委托；自动化、写入/提交和旧 PAT 路径保持关闭。连接高风险变更必须通过安全影响预览；重新测试当前不会发起网络请求，外部连通性仍未验证。</p>
          </div>
        </div>
      </section>
      ...
      <div className="mt-6 grid min-w-0 gap-6 lg:grid-cols-[minmax(280px,.78fr)_minmax(0,1.22fr)]">
        ...
        <section className="min-w-0 rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6">
          ...
          {loading ? <div className="mt-5 space-y-3" role="status" aria-label="正在加载 Git 连接"><div className="h-48 animate-pulse rounded-2xl bg-slate-100" /><div className="h-48 animate-pulse rounded-2xl bg-slate-100" /></div> : connections.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-5 py-9 text-center"><p className="text-sm font-semibold text-slate-700">还没有个人 Git 连接</p><p className="mt-2 text-xs leading-5 text-slate-500">从左侧添加一个连接，保存后可在卡片中生成重新测试预览；当前不会发起网络请求。</p></div> : <div className="mt-5 space-y-4">{connections.map((connection) => <GitConnectionCard key={connection.id} connection={connection} onRemoved={() => { setConnections((current) => current.filter((item) => item.id !== connection.id)); setMessage({ tone: "success", text: "Git 连接已删除。" }); }} onReload={load} />)}</div>}
        </section>
      </div>
    </div>
  );
...
function GitCreateForm({ catalog, onCreated }: { catalog: readonly GitCatalogEntry[]; onCreated: (connection: GitConnection) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage({ tone: "success", text: "已加密保存。密钥输入框已清空；后续变更请在连接卡片中通过安全治理预览管理。当前不会发起网络请求，外部连通性仍未验证。" });
    } catch (error) {

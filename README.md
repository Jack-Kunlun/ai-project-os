...
当前版本状态：`CONTROLLED_PRERELEASE` · `0.5.0-dev.4（受控升级预发布）`

`v0.5.0-dev.1` 的一次性 clean reset 只属于历史生产基线。`v0.5.0-dev.4` 计划通过保留数据的升级通道发布：复用现有 PostgreSQL、secrets、uploads、用户、平台配置和审计数据，不执行卷删除或重新初始化。`.4` 在精确提交 CI、备份、迁移、健康、重启持久性和现场验收完成前，不宣称已经生产部署。

正式稳定版基线：无。已发布的历史生产基线为 `v0.5.0-dev.1`，当前受控升级候选为 `v0.5.0-dev.4`；两者仍是预发布，不是稳定版或 GitHub Latest。`.4` 只有在保留数据升级门禁完成后才能进入生产 tag 通道，旧 `v0.5.0-dev.2`、`v0.5.0-dev.3`、`v0.4.0-dev.1`、`v0.3.0-dev.1`、`v0.2.0-dev.1` 和其他内部开发版本均不得进入该通道。
...
- 当前生产基线为 `0.5.0-dev.1`，`0.5.0-dev.4` 是尚待现场验收的受控升级预发布；两者都不是稳定版或 GitHub Latest。平台管理员只负责平台运营和安全治理，不进入项目、团队或用户工作区；普通用户负责自己的项目和个人连接。个人 MCP 工具发现、管理员净化审核与项目委托/只读授权控制面已开放；旧管理员 Git、旧会员和账号矩阵入口只保留受守卫的兼容跳转。
...
面向局域网外提供服务前，应按[部署安全基线](docs/deployment-security.md)配置 HTTPS、入口限流和可信反向代理，并设置 `AI_PROJECT_OS_SECURE_COOKIES=true` 与实际 HTTPS `AI_PROJECT_OS_PUBLIC_ORIGIN`。仓库提供的 Nginx 示例必须替换域名与证书路径并通过 `nginx -t` 后才能启用。`v0.5.0-dev.1` 的生产工作流仅作为历史 clean reset 记录保留；`v0.5.0-dev.4` 必须经保留数据升级通道验证后才可进入生产。cluster-admin、migrator、runtime、writer 和 inventory-reader 密码若包含 URL 保留字符，需要先进行 URL 编码。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖当前 `0.5.0-dev.4` 受控升级预发布的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准；`0.5.0-dev.4` 仍是预发布，不是稳定版或 GitHub Latest，`v0.5.0-dev.1` 作为历史生产基线保留。

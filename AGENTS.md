...
当前产品已开放页面化多用户/RBAC、邀请与 OIDC、加密凭据、OpenAI/DeepSeek/Qwen/GLM、项目 AI 路由、文件与视觉识别、多 Git 代码仓库、网页来源、本地文件夹、自动抽取、统一向量索引、语义检索、引用式 RAG、持久化自动化、受控动作与审批、远程只读 MCP 工具授权与逐次审批、记忆质量、项目简报和受约束的只读项目智能体。产品说明必须区分真实能力与未实现能力；未通过实现与运行验收的内容不得在 README、产品文案或 UI 中宣称可用。
...
- Action Engine 只能调用显式白名单中的内置能力，或经管理员验证、项目 Owner 逐项授权且每次单独审批的远程只读 MCP 工具。动作必须绑定项目、规范化输入、输入指纹、能力风险和策略快照；MCP 动作还必须绑定授权、工具定义、网络与凭据指纹。审批记录、策略修订、工具定义快照和审计记录不可变。项目 Owner 才能修改策略、授权工具或审批，租约过期和任何快照漂移必须失败关闭且不得自动重试外部读取。
- MCP 仅支持当前 Streamable HTTP 远程传输；禁止 stdio、本地子进程、旧式 HTTP+SSE、执行中交互输入、自动执行和任何写操作。远端 annotations 必须视为未受信提示；接入方仍需使用可信服务和最小权限只读凭据。工具结果不得自动进入记忆、RAG 或模型上下文。
...
每次行为变化运行与风险相称的聚焦检查。当前版本发布验收必须包含：Prisma 生成与校验、完整迁移链、类型检查、Lint、测试、生产构建、真实 PostgreSQL Action Engine 与 MCP 授权/审批/失败关闭闭环、Docker Compose `postgres/migrate/app/worker` 状态、认证后页面 smoke，以及按钮文字居中和共享 Header 一致性。真实外部模型、Git、OIDC 或第三方 MCP 服务未配置时，必须明确说明对应连接未做现场验证；MCP 协议联调应使用隔离的只读测试服务，不得用静态检查替代。

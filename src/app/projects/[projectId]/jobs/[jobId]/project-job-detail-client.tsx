...
export function ProjectJobDetailClient({ username }: { username: string }) {
  ...
  const backHref = fromNotifications ? `/notifications?view=${notificationFilter}${notificationPageCursor ? `&cursor=${encodeURIComponent(notificationPageCursor)}` : ""}${notificationFocusId ? `&focus=${encodeURIComponent(notificationFocusId)}` : ""}` : job ? parentRoute(projectId, job.kind) : `/projects/${projectId}/governance#task-runs`;

...
凭据只写入未提交的部署配置或受控密钥管理，不写入源码、文档、URL 或日志。`AI_PROJECT_OS_PUBLIC_ORIGIN` 必须是浏览器实际访问的规范 origin，不能填容器内部地址。当前生产 job 只精确允许目标 `v0.6.0-dev.6`，并要求显式选择实际运行的 `v0.5.0-dev.1` 或 `v0.5.0-dev.4` 源版本；其他标签均失败关闭，不要因为配置了 GitHub Secret 就绕过标签 CI、备份恢复、旧库预检和停写切换门禁。

...
本工具用于单节点实例的 PostgreSQL、凭据主密钥卷、上传卷和主机恢复配置备份。它可以独立于发布入口安装；当前部署链支持 `v0.5.0-dev.1` clean-reset、历史 `v0.5.0-dev.2` 和当前 `v0.5.0-dev.3` preserve-data 的精确备份命名，仍拒绝旧 `v0.4.0-dev.1`、`v0.3.0-dev.1`、`v0.2.0-dev.1` 和其他未批准的预发布标签。
...
`v0.5.0-dev.3` 的 `preserve-deploy` 使用 `pre-deploy-to-v0.5.0-dev.3` 命名，备份完成后只替换 app/worker，不启动数据库迁移或初始化服务，不删除 PostgreSQL 容器/卷。`.1` recovery 接受这份 source appVersion 为 `.1` 的部署前备份；`.3` future recovery 仍要求 manifest appVersion 与目标标签精确一致。历史 `.2` 备份命名继续保留在恢复白名单中。
...
- `v0.5.0-dev.3` preserve-data 部署器沿用同一 stopped-writer 备份门禁，但只允许目标名 `pre-deploy-to-v0.5.0-dev.3`；成功后校验 PostgreSQL 容器、卷身份、106 条 migration ledger 和关键数据计数均未减少。历史 `.2` 目标仍可由备份/恢复脚本识别，但不再进入新的生产部署入口。

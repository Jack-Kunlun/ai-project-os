...
AI Project OS V2.3.0 是一套可在本机部署的项目长期记忆与只读项目智能工作台。它把人工资料和多个 GitHub 仓库整理成可追溯记忆，并通过 Dashboard、治理与审核中心以及可恢复任务工作流，提供页面化的模型配置、自动抽取、增量向量索引、语义检索、带引用 RAG、项目状态简报和受约束的只读智能体。
...
| 项目 | 独立项目管理页；支持搜索、创建、软归档、恢复和受限 JSON 导出，并从项目卡进入五个项目工作区 |
| 个人中心 | 查看账户、会话和活动时间；修改登录名；验证当前密码后轮换密码并撤销全部会话 |
| 模型供应商 | 页面配置和测试 OpenAI、DeepSeek、Qwen、GLM；API Key 加密保存且永不回显 |
| 能力路由 | 每个项目分别选择自动抽取、向量索引和引用式生成所使用的供应商与模型；保存前预览影响并保留无密钥审计记录 |
| 项目资料 | 保存人工原文、来源、时间和 SHA-256；条目必须引用同项目资料中的精确摘录 |
| GitHub 多仓库 | 页面连接多个仓库；只读 fine-grained PAT；按分支与目录冻结 commit；支持项目级同步、逐目标结果和安全变更清单 |
| 自动抽取 | 从明确选择的资料抽取 decision、progress、issue、risk；结构和精确原文摘录验证失败时整次拒绝 |
| 人工审核 | 接受或驳回 AI 候选；所有动作保留模型、作业、证据和修订记录 |
| 统一向量记忆 | 对人工资料、当前仓库资料生成和当前项目代码快照确定性分块；支持增量复用或全量构建，完成后原子切换活动索引 |
...
| 可恢复作业 | GitHub 与 AI 操作持久化为带 attempt/lease 的任务；区分失败与未知结果，未知结果只能按专用审计规则人工收口，不自动重试 |
| 治理与审核 | 统一审核两类 AI 候选、查看异常任务与 GitHub 风险、核对模型路由历史，并按 7/30/90 天查看调用与 Token 用量 |
| 安全导出 | 在一致性事务中生成最大 20 MiB 的项目 JSON，记录字节数与 SHA-256；排除系统凭据、向量和原始任务载荷 |
...
8. 进入“治理与审核”处理候选和未知结果，核对模型路由历史与 7/30/90 天模型用量。
9. 进入“项目智能体”，确认本次传输范围后生成项目当前状态简报，或让智能体按固定只读工具调查一个项目问题。
...
- 切换生成模型只影响后续任务，历史结果及其供应商、模型来源保持不变；切换向量路由必须明确确认索引重建影响。
- 索引发布会核对构建开始时的活动指针、路由、供应商状态和输入清单；任一条件变化都会拒绝发布并保留上一代完整索引。
- 归档会保留不可变生命周期审计；运行中或待人工收口任务会阻止归档，归档项目拒绝写入和新活动任务。
- 当前版本不提供项目永久删除 API 或页面入口；候选 Source 只有在未被 Item 引用时才允许删除。供应商凭据与全局连接独立保存。
...
- 模型用量不维护供应商价格、折扣或缓存计费规则，因此不估算费用；最终金额以供应商账单为准。
- 受限 JSON 是内容核对与迁移文件，不是数据库备份；它不恢复凭据、向量、扫描中间账本或完整运行状态。
- 已有 V1 CLI 和历史运行合同继续保留用于兼容与审计，但 V2.3.0 的常规配置和操作入口是网页。
...
# 真实 PostgreSQL 项目归档、恢复、审计与安全导出
PROJECT_LIFECYCLE_POSTGRES_GATE=1 pnpm exec tsx --test test/project-lifecycle-postgres.test.ts
...
- `/projects`：独立项目列表、搜索、创建、归档、恢复、安全导出和项目工作区快捷入口。
...
- `/projects/:projectId/governance`：候选审核、异常任务、模型用量、索引风险和路由变更审计。
...
- `/api/projects/:projectId/governance/*`：治理概览、审核队列、任务、路由历史和模型用量。
- `/api/projects/:projectId/lifecycle`、`/export`：带乐观并发的软归档/恢复和受审计安全 JSON 导出。
...
V1 CLI 手册和历史合同保留在 `docs/`，仅用于兼容、审计和理解旧数据结构。它们不覆盖本 README 描述的当前 V2.3.0 页面能力。

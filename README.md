...
AI Project OS V5.1.0 是一套可本地部署的项目长期记忆、时态项目世界模型、只读项目智能、受控动作与证据驱动运营工作台。它把人工资料、文件、网页、多个 Git 服务中的代码仓库和经人工纳入的 MCP 只读结果整理为有来源、有版本、有审核状态的项目证据，再把当前有效事实、人工关系、替代链、冲突和项目计划合成为可复现的当前状态，并提供语义检索、引用式 RAG、项目状态简报、受约束的只读智能体、持久化自动化、动作审批、不可变审计、记忆质量治理和团队权限。
...
- V5.1.0 当前提供一个由旧数据迁移得到的默认工作区；数据模型支持多个工作区，但页面尚未提供工作区创建和切换。
...
V1 CLI 手册和历史运行合同继续保留用于兼容与审计，但不覆盖 V5.1.0 的页面能力。当前能力、限制和使用方式以本 README、页面指南和操作手册为准。
